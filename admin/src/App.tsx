...
import { BarChart3, Building2, DoorOpen, LogOut, Menu, PanelLeftClose, ShieldCheck, X, Settings, Trophy } from 'lucide-react';
...
import CanopyCompetitionPage from './pages/CanopyCompetitionPage';
...
function Shell({ profile }: { profile: AdminProfile }) {
  ...
  const links=[['/',BarChart3,'Overview'],['/canopy-competition',Trophy,'Canopy Run'],['/brands',Building2,'Partners'],['/exhibits',DoorOpen,'Exhibits'],['/settings',Settings,'Settings']] as const;
  return <div className="app-shell"><button className="mobile-menu" onClick={()=>setOpen(true)} aria-label="Open navigation"><Menu/></button>
    ...
    </aside><main className="main-content"><Routes><Route path="/" element={<DashboardPage/>}/><Route path="/canopy-competition" element={<CanopyCompetitionPage/>}/><Route path="/brands" element={<BrandsPage/>}/><Route path="/exhibits" element={<ExhibitsPage/>}/><Route path="/settings" element={<SettingsPage/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes></main></div>;
}
