...
export const MuseumInfoModal:React.FC<{point:MuseumInfoPoint;onClose:()=>void}>=({point,onClose})=>{
  ...
  return <div className="museum-modal-backdrop animate-fade-in" onMouseDown={event=>event.target===event.currentTarget&&close()}>
    <section className="museum-modal" role="dialog" aria-modal="true" aria-labelledby="info-title">
      <header className="museum-modal__header"><div><p className="museum-modal__eyebrow">{isGuide?'Visitor guide':'Partnerships'}</p><h2 id="info-title">{point.title}</h2></div><button className="museum-modal__close" onClick={close} aria-label="Close"><X size={20}/></button></header>
      <div className="museum-modal__body">
        {isGuide ? <>
          ...
          <div className="guide-steps"><div><Move/><span><strong>Explore</strong>Use the joystick or WASD to move. Drag the view to look around.</span></div><div><Eye/><span><strong>Inspect</strong>Approach art or exhibits and select Inspect. Desktop players can press E.</span></div><div><Gift/><span><strong>Collect</strong>Find three gallery rewards. They remain saved in your museum passport.</span></div><div><ArrowUpCircle/><span><strong>Move freely</strong>Run, jump, talk to nearby visitors, or return to this book whenever you need help.</span></div></div>
          ...
          <div className="museum-modal__summary"><div className="museum-modal__summary-row"><span>Partnership contact</span><strong>+1 409 422 9714</strong></div><div className="museum-modal__note"><span>Send a WhatsApp message to discuss advertising availability, campaign ideas, or a museum partnership.</span></div></div>
