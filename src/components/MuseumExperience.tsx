...
import { MuseumInfoModal } from './MuseumInfoModal';
...
export interface MuseumExperienceProps {
  visitorName: string;
  avatarColor: string;
  onExitToReception: () => void;
}

export const MuseumExperience: React.FC<MuseumExperienceProps> = ({
  visitorName,
  avatarColor,
  onExitToReception
}) => {
  ...
  const [discoveredCodes, setDiscoveredCodes] = useState<Record<BrandKey, DiscoveredCoupon | null>>(() => {
    ...
    return {
      ...
      clayrent: null,
      filedcrews: null
    } as Record<BrandKey, DiscoveredCoupon | null>;
  });
  ...
  const huntStartTime = useRef<number>(Date.now());
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (elapsedTime - lastProximityCheck > 0.08) {
        ...
        // Check proximity to South Entrance Exit Portal (z > 14.2 within entrance corridor)
        const nearExitPortal = controller.position.z >= 14.2 && Math.abs(controller.position.x) <= 2.2;
        setIsNearExit(nearExitPortal);

        // Close-range discovery with strict line-of-sight checks so walls block prompts
        ...
        for (const ex of EXHIBITS) {
          ...
          if (dist < minDistance) {
            // Verify direct line-of-sight so exhibits in other rooms are not detected through walls
            if (museumScene.hasLineOfSight(controller.position.x, controller.position.z, ex.position[0], ex.position[2])) {
              minDistance = dist;
              closest = ex;
            }
          }
        ...
        for (const artwork of museumScene.artworks) {
          ...
          if (distance < artworkDistance) {
            // Must be standing on the visible front side of the wall, not behind it in another room
            let inFront = true;
            if (artwork.faceDir === 'south' && controller.position.z < artwork.position[1]) inFront = false;
            if (artwork.faceDir === 'north' && controller.position.z > artwork.position[1]) inFront = false;
            if (artwork.faceDir === 'east' && controller.position.x < artwork.position[0]) inFront = false;
            if (artwork.faceDir === 'west' && controller.position.x > artwork.position[0]) inFront = false;

            if (inFront && museumScene.hasLineOfSight(controller.position.x, controller.position.z, artwork.position[0], artwork.position[1])) {
        ...
        for (const point of museumScene.infoPoints) {
          ...
          if (distance < interactionRadius && distance < infoDistance) {
            // Verify line-of-sight so signs/artifacts in adjacent corridors aren't detected through walls
            if (museumScene.hasLineOfSight(controller.position.x, controller.position.z, point.position[0], point.position[1])) {
    ...
    <div className="game-viewport bg-black font-sans select-none">
      ...
      {isVictoryOpen && (
        <VictoryModal
          ...
          visitorName={visitorName}
          durationSeconds={Math.max(1, Math.round((Date.now() - huntStartTime.current) / 1000))}
        />
      )}
    </div>
  );
